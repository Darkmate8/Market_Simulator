    ...
    for step in range(200):
        ...
        for index_bot in range(len(inventory)):
            ...
            if decision == "yes":
                if chosen_action == "sell" and inventory[index_bot] > 0:
                    sellers.append(index_bot)
                elif chosen_action == "buy" and money[index_bot] >= price: 
                    buyers.append(index_bot)
            
            
        if len(sellers) >= len(buyers):
            iteration_length = len(sellers)
        elif len(buyers) > len(sellers):
            iteration_length = len(buyers)
        ...
        for i in range(iteration_length):
            ...
            if len(sellers) > 0 and len(buyers) > 0:
                    execute = True
                    temp_sell_bot = sellers[0]
                    temp_buy_bot = buyers[0]
                    seller_bot += temp_sell_bot 
                    buyer_bot += temp_buy_bot
                    
            if execute == True:
                inventory[seller_bot] -= 1
                money[seller_bot] += price
                total_sells += 1

                inventory[buyer_bot] += 1
                money[buyer_bot] -= price
                total_buys += 1
